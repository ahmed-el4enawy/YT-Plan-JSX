...
export default function CaptionGenerator() {
  ...
  const [serverCaptions, setServerCaptions] = useState(null);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const data = await res.json();
          ...
          // Fetch transcript
          try {
            const trRes = await fetch("http://localhost:5000/api/transcript/" + id);
            if (trRes.ok) {
              const trData = await trRes.json();
              if (trData && trData.segments) {
                const mapped = trData.segments.map((seg, i) => ({
                  id: i + 1,
                  text: seg.text,
                  startTime: seg.start,
                  endTime: seg.end
                }));
                setServerCaptions(mapped);
              }
            }
    ...
    <div style={{ display: "flex", height: "calc(100vh - 48px)", gap: "24px", overflow: "hidden", margin: "-1.5rem -2rem", padding: "1.5rem 2rem", background: "#0D0E12" }}>
      ...
      <div className="custom-scrollbar" style={{ width: "360px", flexShrink: 0, display: "flex", flexDirection: "column", background: "#14151A", borderRadius: "12px", border: "1px solid #1E2028", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid #1E2028", padding: "0 16px" }}>
          {["Captions", "Text", "Animation"].map((tab) => (
            <div 
              key={tab} 
              onClick={() => { setActiveTab(tab); if(tab === "Text") setActiveSubTab("Basic"); }}
              style={{ padding: "16px 12px", fontSize: "13px", fontWeight: activeTab === tab ? 600 : 500, color: activeTab === tab ? "var(--color-text-info)" : "var(--color-text-secondary)", borderBottom: activeTab === tab ? "2px solid var(--color-text-info)" : "2px solid transparent", cursor: "pointer" }}
            >
              {tab}
            </div>
          ))}
        ...
        {/* Sub Navigation for Text Tab */}
        {activeTab === "Text" && (
          <div style={{ display: "flex", padding: "12px 16px", gap: "8px" }}>
            {["Basic", "Templates", "Bubble", "Effects"].map((tab) => (
              <div 
                key={tab} 
                onClick={() => setActiveSubTab(tab)}
                style={{ flex: 1, textAlign: "center", padding: "6px 0", fontSize: "12px", fontWeight: 500, background: activeSubTab === tab ? "#1E2028" : "transparent", color: activeSubTab === tab ? "#FFF" : "var(--color-text-secondary)", borderRadius: "4px", cursor: "pointer" }}
              >
                {tab}
              </div>
            ))}
        ...
        <div style={{ padding: "16px 16px 24px" }}>
                              ...
                              {/* ========== CAPTIONS TAB ========== */}
          {activeTab === "Captions" && (
            <div style={{ height: "100%", margin: "-16px", flex: 1, display: "flex", flexDirection: "column" }}>
                                          <TranscriptEditor 
                initialCaptions={serverCaptions} 
                onCaptionsChange={(newCaps) => setServerCaptions(newCaps)} 
              />
            </div>
          )}
          ...
          {activeTab === "Text" && (
            <>
              ...
              {activeSubTab === "Basic" && (
                <>
                  <SectionHeader title="Advanced Typography" />
                  <ControlRow label="Font Family">
                    <select style={{ background: "#0A0B0E", color: "#FFF", border: "1px solid #1E2028", borderRadius: "4px", padding: "4px 8px", fontSize: "12px", outline: "none", cursor: "pointer", width: "100%" }}>
                  ...
                  <ControlRow label="Letter Spacing">
                    <SliderControl value={0} min={-10} max={20} onChange={() => {}} unit="px" />
                  </ControlRow>
                  ...
                  <ControlRow label="Color">
                    <div style={{ width: "60px", height: "24px", background: "#000", border: "1px solid #333", borderRadius: "4px", cursor: "pointer" }}></div>
                  </ControlRow>
              ...
              {activeSubTab === "Bubble" && (
                <div style={{ padding: "40px 0", textAlign: "center", color: "var(--color-text-tertiary)", fontSize: "13px" }}>
                  Select a bubble style to apply to text.
                </div>
              )}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            {status === "completed" && jobId && (
              <button 
                onClick={async () => {
                  setStatus("processing");
                  try {
                    await fetch("http://localhost:5000/api/reprocess/" + jobId, {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ segments: serverCaptions })
                    });
                    pollStatus(jobId);
                  } catch(e) {
                    setStatus("error");
                    setErrorMsg("Failed to re-render");
                  }
                }}
                style={{ 
        ...
        <div style={{ flex: 1, background: "#0A0B0E", borderRadius: "12px", border: "1px solid #1E2028", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
                    {file || (status === "completed" && jobId) ? (
            <video 
              key={status === "completed" ? "result" : "preview"}
              src={status === "completed" && jobId ? "http://localhost:5000/api/download/" + jobId : videoUrl} 
              controls 
